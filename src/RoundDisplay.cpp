 ...
 * @return
 */
RoundDisplay::~RoundDisplay() noexcept
{
  Destroy();
}

void
RoundDisplay::Create(PixelSize size, UI::TopWindowStyle style)
{
  SingleWindow::Create(title, size, style);
}

void
RoundDisplay::Initialise()
{
...
//                     200);
  ...
  traffic_gauge.Set(new GaugeFLARM(CommonInterface::GetLiveBlackboard(), GetLook().flarm_gauge_round_display));

  GaugeFLARM *widget = (GaugeFLARM *)traffic_gauge.Get();
  PixelRect rc = GetClientRect();
  traffic_gauge.Move(rc);
  traffic_gauge.Show();
  traffic_gauge.SetFocus();
  widget->Raise();
  top_widget = widget;
//
//  rc = GetMainRect();
...
void
RoundDisplay::InitialiseConfigured()
{
  ...
  const InfoBoxLayout::Layout ib_layout = InfoBoxLayout::Calculate(rc, ui_settings.info_boxes.geometry);
...
void
RoundDisplay::Deinitialise() noexcept
{
...
void
RoundDisplay::ReinitialiseLayout_vario(const InfoBoxLayout::Layout &layout) noexcept
{
  if (!layout.HasVario()) {
    vario.Clear();
    return;
...
void
RoundDisplay::ReinitialiseLayoutTA(PixelRect rc,
...
void
RoundDisplay::ReinitialiseLayout() noexcept
{
  if (map == nullptr)
...
void
RoundDisplay::ReinitialiseLayout_flarm(PixelRect rc,
...
void
RoundDisplay::FinishStartup() noexcept
{
...
void
RoundDisplay::BeginShutdown() noexcept
{
  timer.Cancel();
...
void
RoundDisplay::SuspendThreads() noexcept
{
  if (map != nullptr)
    map->SuspendThreads();
}
...
void
RoundDisplay::ResumeThreads() noexcept
{
  if (map != nullptr)
    map->ResumeThreads();
}
...
void
RoundDisplay::SetDefaultFocus() noexcept
{
...
void
RoundDisplay::FlushRendererCaches() noexcept
{
  if (map != nullptr)
    map->FlushCaches();
}
...
void
RoundDisplay::FullRedraw() noexcept
{
  if (map != nullptr)
    map->FullRedraw();
}
...
void
RoundDisplay::OnResize(PixelSize new_size) noexcept
{
...
void
RoundDisplay::OnSetFocus() noexcept
{
...
void
RoundDisplay::StopDragging() noexcept
{
  if (!dragging)
    return;
...
void
RoundDisplay::OnCancelMode() noexcept
{
...
bool
RoundDisplay::OnMouseDown(PixelPoint p) noexcept
{
...
bool
RoundDisplay::OnMouseUp(PixelPoint p) noexcept
{
...
bool
RoundDisplay::OnMouseDouble(PixelPoint p) noexcept
{
...
bool
RoundDisplay::OnMouseMove(PixelPoint p, unsigned keys) noexcept
{
...
bool
RoundDisplay::OnKeyDown(unsigned key_code) noexcept
{
...
inline void
RoundDisplay::LateInitialise() noexcept
{
  if (late_initialised)
    return;
...
void
RoundDisplay::RunTimer() noexcept
{
  LateInitialise();
...
void
RoundDisplay::OnGpsNotify() noexcept
{
...
void
RoundDisplay::OnCalculatedNotify() noexcept
{
  UIReceiveCalculatedData();
}

void
RoundDisplay::OnRestorePageNotify() noexcept
{
...
void
RoundDisplay::OnDestroy() noexcept
{
  timer.Cancel();
...
void
RoundDisplay::OnPaint(Canvas &canvas) noexcept
{
...
void
RoundDisplay::SetFullScreen(bool _full_screen) noexcept
{
...
void
RoundDisplay::SetTopography(TopographyStore *topography) noexcept
{
  if (map != nullptr)
...
void
RoundDisplay::SetComputerSettings(const ComputerSettings &settings_computer) noexcept
{
  if (map != nullptr)
...
void
RoundDisplay::SetMapSettings(const MapSettings &settings_map) noexcept
{
  if (map != nullptr)
...
void
RoundDisplay::SetUIState(const UIState &ui_state) noexcept
{
  if (map != nullptr) {
...
void
RoundDisplay::DeferredRestorePage() noexcept
{
  if (restore_page_pending)
    return;
...
void
RoundDisplay::KillWidget() noexcept
{
  if (widget == nullptr)
    return;
...
void
RoundDisplay::KillTopWidget() noexcept
{
  if (top_widget == nullptr)
    return;
...
void
RoundDisplay::SetTopWidget(Widget *_widget) noexcept
{
  ...
  PixelRect main_rect = GetClientRect();
  const PixelRect top_rect = GetTopWidgetRect(main_rect,
                                              top_widget);
  if (top_widget != nullptr) {
...
void
RoundDisplay::KillBottomWidget() noexcept
{
  if (bottom_widget == nullptr)
    return;
...
void
RoundDisplay::SetBottomWidget(Widget *_widget) noexcept
{
...
void
RoundDisplay::SetWidget(Widget *_widget) noexcept
{
...
Widget *
RoundDisplay::GetFlavourWidget(const TCHAR *flavour) noexcept
{
...
bool
RoundDisplay::IsMenuButtonEnabled(unsigned idx) noexcept
{
...
void
RoundDisplay::UpdateVarioGaugeVisibility() noexcept
{
...
void
RoundDisplay::UpdateGaugeVisibility() noexcept
{
  UpdateVarioGaugeVisibility();
...
void
RoundDisplay::UpdateTrafficGaugeVisibility() noexcept
{
...
RoundDisplay::GetProjection() const noexcept
{
  AssertThread();
  assert(map != nullptr);
...
void
RoundDisplay::ToggleSuppressFLARMRadar() noexcept
{
  suppress_traffic_gauge = !suppress_traffic_gauge;
}

void
RoundDisplay::ToggleForceFLARMRadar() noexcept
{
